...
function warnDb() {
  if (!dbWarned && !isMongo) {
    console.warn('\nвљ пёЏ  Р’РќРРњРђРќРР•: РСЃРїРѕР»СЊР·СѓРµС‚СЃСЏ С„Р°Р№Р»РѕРІР°СЏ Р‘Р” (game.db).');
    console.warn('   РќР° Render РґР°РЅРЅС‹Рµ РїСЂРѕРїР°РґСѓС‚ РїРѕСЃР»Рµ РїРµСЂРµР·Р°РїСѓСЃРєР°!');
    console.warn('   РќР°СЃС‚СЂРѕР№ MONGO_URI РІ РїРµСЂРµРјРµРЅРЅС‹С… РѕРєСЂСѓР¶РµРЅРёСЏ Render.\n');
    dbWarned = true;
  }
}
...
const userSchema = new mongoose.Schema({
  ...
  avatar: { type: String, default: 'рџЊ±' },
...
async function mongoQuery(sql, params) {
  if (sql.includes('INSERT INTO users')) {
    ...
    return { rows: [{ id: user._id.toString(), nickname: user.nickname, wins: user.wins, losses: user.losses, coins: user.coins || 0, avatar: user.avatar || 'рџЊ±', clan: user.clan || '', friends: user.friends || [], unlocked_plants: user.unlocked_plants || [1, 2, 3], unlocked_zombies: user.unlocked_zombies || [1, 2, 3] }] };
  }
  ...
  if (sql.includes('SELECT') && sql.includes('FROM users') && sql.includes('ORDER BY')) {
    ...
    return { rows: users.map(u => ({ id: u._id.toString(), nickname: u.nickname, wins: u.wins, losses: u.losses, is_admin: u.is_admin || false, is_banned: u.is_banned || false, role: u.role || 'player', created_at: u.createdAt })) };
  }
...
function fileQuery(sql, params) {
  ...
  if (sql.includes('INSERT INTO users')) {
    ...
    fileDb.users.push({ id, nickname: params[0], password_hash: params[1], wins: 0, losses: 0, coins: 0, avatar: '', clan: '', friends: [], friend_requests: [], is_admin: false, is_banned: false, role: 'player', unlocked_plants: [1, 2, 3], unlocked_zombies: [1, 2, 3], created_at: new Date().toISOString() });
    saveFileDb();
    return { rows: [{ id, nickname: params[0], wins: 0, losses: 0, coins: 0, avatar: '', clan: '', friends: [], friend_requests: [], is_admin: false, is_banned: false, role: 'player', unlocked_plants: [1, 2, 3], unlocked_zombies: [1, 2, 3] }] };
  }
  ...
  if (sql.includes('SELECT') && sql.includes('FROM users') && sql.includes('ORDER BY')) {
    return { rows: fileDb.users.map(u => ({ id: u.id, nickname: u.nickname, wins: u.wins, losses: u.losses, is_admin: u.is_admin || false, is_banned: u.is_banned || false, role: u.role || 'player', created_at: u.created_at })) };
  }
